...
class Video extends \app\index\controller\Base
{
    public function _initialize()
    {
        parent::_initialize();
        $this->assign('navSelTag', 'video');
    }

    public function index()
    {
        if (is_mobile_request() === true) {
            $this->redirect(config('global_config.mobile_domain') . 'video', 302);
            exit;
        }
        ...
        if ($cid > 0) {
            $categoryinfo = model('VideoCategory')->where('id', $cid)->find();
        ...
        foreach ($list as $key => $value) {
            ...
            $list[$key]['link_url'] = $value['link_url'] == '' ? url('index/video/show', ['id' => $value['id']]) : $value['link_url'];
        }
        $options = model('VideoCategory')->getCache();
        $options1 = $options2 = [];
        $counter = 1;
    ...
    public function show()
    {
        ...
        $prev = model('Video')
            ...
            ->find();
        if ($prev !== null) {
            $prev['link_url'] = $prev['link_url'] == '' ? url('index/video/show', ['id' => $prev['id']]) : $prev['link_url'];
        }
        ...
        $next = model('Video')
            ...
            ->find();
        if ($next !== null) {
            $next['link_url'] = $next['link_url'] == '' ? url('index/video/show', ['id' => $next['id']]) : $next['link_url'];
        }
        $options = model('VideoCategory')->getCache();
    ...
    {
        $list = model('Video')->where('id', 'neq', $id)->limit(10)->order('click desc')->select();
        $thumb_id_arr = $thumb_arr = [];
        foreach ($list as $key => $value) {
            $value['thumb'] > 0 && ($thumb_id_arr[] = $value['thumb']);
        }
