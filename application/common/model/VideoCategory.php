...
class VideoCategory extends \app\common\model\BaseModel
{
    ...
    protected static function init()
    {
        self::event('after_write', function () {
            cache('cache_video_category', null);
        });
    ...
    {
        // if (false === ($data = cache('cache_video_category'))) {
        $data = $this->order('sort_id desc,id asc')->column('id,name');
        // cache('cache_video_category', $data);
        // }
        if ($id != 0) {
            return $data[$id];
        }
